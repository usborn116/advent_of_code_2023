str = 'Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
class Game
    ...
    def initialize(game, matches)
        @game_id = game.first.scan(/\d+/).first.to_i#split(' ')[1]
        p @game_id
        @matches = matches.map{|m| create_match(m)}
        p @matches
    end
    ...
    def all_possible?
        @matches.all? {|m| m.possible? }
    end

    def maxes
        red = @matches.map(&:red).max
        blue = @matches.map(&:blue).max
        green = @matches.map(&:green).max
        red * blue * green
    end
...
class Match
    ...
    def initialize(colors)
        @blue = colors.select{|c| c.split(' ')[1] == 'blue'}&.first&.split(' ')&.first.to_i || 0
        @red = colors.select{|c| c.split(' ')[1] == 'red'}&.first&.split(' ')&.first.to_i || 0
        @green = colors.select{|c| c.split(' ')[1] == 'green'}&.first&.split(' ')&.first.to_i || 0
    end

    def possible?
        @red <= 12 && @green <= 13 && @blue <= 14# ? @game_id : 0
    end
...
def parser(text)
    ...
    ans = 0
    games = text.split("\n")
    games = games.map{|g| g.split(': ')}
    games = games.map{|s| [[s[0]], s[1].split('; ')]}
    games = games.map{|g| [g[0], g[1].map{|p| p.split(', ')}]}
    games = games.map {|g| Game.new(g[0], g[1])}
    games.each {|g| ans += g.maxes}
    p ans
end
